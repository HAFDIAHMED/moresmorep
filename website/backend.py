...
LIMIT = 10
HEADERS = ['Timestamp (UTC)', 'Position', 'Name', 'Address', 'Location', 'IP']
...
def ensure_workbook():
    """Create the Excel file with a header row if it doesn't exist yet."""
    if XLSX.exists():
        return
    wb = Workbook()
    ws = wb.active
    ...
    ws.append(HEADERS)
    # Sensible column widths so the file opens cleanly in Excel
    widths = {'A': 22, 'B': 10, 'C': 28, 'D': 60, 'E': 28, 'F': 18}
